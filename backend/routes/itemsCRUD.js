...
router.post("/:supermarketId/item-add", async (req, res) => {
    try {
        ...
        const { item_type, item_name, price, available_quantity, description } = req.body;
        ...
        console.error(error);
        res.status(500).json({ error: "An error occurred while adding the item to the supermarket" });
    }
});
...
router.put("/:supermarketId/item-update/:itemId", async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).json({ error: "An error occurred while updating the item in the supermarket" });
    }
});
...
router.delete("/:supermarketId/item-delete/:itemId", async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).json({ error: "An error occurred while removing the item from the supermarket" });
    }
});
...
router.get("/:supermarketId/items", async (req, res) => {
    try {
        ...
        res.json(supermarket.items);
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: "An error occurred while fetching items from the supermarket" });
    }
});
