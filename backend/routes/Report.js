...
// Create complaint
router.post("/quality-report-create", async (req, res) => {
    const {
        driver_id,
        market_name,
        district,
    ...
    try {
        ...
        if (!driver_id || !market_name || !district ||  !issue_type || !description) {
            return res.status(400).json({ message: 'All fields are required' });
        }

        const existingemp = await employeeSchema.findById(driver_id);
...
router.put("/quality-report-update/:id", async (req, res) => {
      const {driver_id, market_name, district, branch, issue_type, description } = req.body;
      const { id } = req.params;
