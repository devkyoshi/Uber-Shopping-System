...
router.put("/update-task/:taskId", async (req, res) => {
    try {
        ...
        const updatedTask = await Task.findByIdAndUpdate(taskId, { driver_id, task_status, route }, { new: true });
        if (!updatedTask) {
...
router.post("/:taskId/add-order", async (req, res) => {
    try {
        const { taskId } = req.params;
        const { order_id, status, pickup_image_url, delivered_image_url, delivered_time, pickedup_time } = req.body;
        ...
        console.error(error);
        res.status(500).json({ error: "An error occurred while adding order details to the task" });
    }
});
...
router.put("/:taskId/update-order/:orderId", async (req, res) => {
    try {
        ...
        const { status, pickup_image_url, delivered_image_url, delivered_time, pickedup_time } = req.body;
        ...
        order.delivered_image_url = delivered_image_url || order.delivered_image_url;
        ...
        console.error(error);
        res.status(500).json({ error: "An error occurred while updating order details" });
    }
});
...
router.delete("/:taskId/remove-order/:orderId", async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).json({ error: "An error occurred while removing order from the task" });
    }
});
...
router.get("/:taskId/get-all-orders", async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).json({ error: "An error occurred while fetching all orders for the task" });
    }
});
